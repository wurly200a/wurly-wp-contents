...
#+POSTID: 1996
#+ORG2BLOG:
#+DATE: [2025-09-27 Sat 08:44]
#+OPTIONS: toc:nil num:nil todo:nil pri:nil tags:nil ^:nil
#+CATEGORY: Emacs
#+TAGS: 
#+DESCRIPTION:
#+TITLE: Ubuntu 24.04 に Emacs 30.2 (with pgtk) をインストール
...
インストール環境は下記です。
今回は、最初にビルドしたのがNativeのUbuntu環境を使いましたが、直近の数バージョンで、バージョンアップがある毎にNative/WSL両方の環境でもビルドしていますので、Native/WSLどちらでも同じ手順で実行できるかと思います。

 - ハードウェア : ASUS Chromebox3 i7-8550U 32GB RAM, NVMe SSD
 - OS : Ubuntu24.04
...
libgccjit は 14 を使用しました。
今回、imagemagick, libmagickwand-dev, libmagickcore-dev, libtree-sitter-dev を追加しました。
また、現在は libm17n-dev は不要のようです(HarfBuzzが標準とのこと)。
gtk4-dev では使用不可です。

(gtk3-devなし、gtk4-devありのとき)
#+begin_src bash
...
cd emacs-30.2
./autogen.sh
CFLAGS='-I/usr/lib/gcc/x86_64-linux-gnu/14/include -L/usr/lib/gcc/x86_64-linux-gnu/14' ./configure --with-pgtk --with-native-compilation --without-pop --with-mailutils --with-imagemagick --with-tree-sitter
...
* Ubuntu 24.04 で pure-GTK にて起動しない問題(WSL)

(Emacs 30.1のときの対応内容の転記です)
...
Ubuntu 24.04 でも Emacs 30.2 のビルドおよび起動が確認できました。
