...
** Step 7: Holder Pods の無効化（v1.16 に備えて）

=csi-*plugin-holder-*= Pod が存在する場合、将来の v1.16 アップグレード前に無効化が必要です。

#+begin_src shell
kubectl -n rook-ceph get pods | grep holder
#+end_src

存在する場合は values.yaml を更新して Helm upgrade を実行します。
...
| Step 6   | =HEALTH_OK= 確認                                |                             |
| Step 7   | holder pods 無効化                            | v1.16 アップグレード前に必須 |
...
** Ceph クラスタが HEALTH_WARN / HEALTH_ERR の場合

Toolbox を使って Ceph の状態を直接確認します。
